...
The proportion of NAs within the "Step" variable is `r proportion`%
...
```{r}
hist(total_steps$steps, col = "green", breaks = 10, main = "Frequency of Total Number of Steps",
...
The maean of steps is **`r mean_steps`** and the median is **`r median_steps`**
...
```{r}
library(ggplot2)
ggplot(Average_by_interval, aes(interval, steps)) + geom_line(color = "red") + theme_bw() +
...
```{r}
hist(total_steps2$steps, breaks = 10, col = "lightblue", main = "Frequency of Total Number of Steps",
     xlab = "Total Number of Steps", ylab = "Frequency")
```
...
ggplot(Average_by_interval3, aes(Interval, Steps, color = Weekday)) + geom_line(size = 1) + 
